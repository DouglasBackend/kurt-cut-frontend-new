...
import { getTrackTypeForElementType, areTrackTypesCompatible } from "./compatibility";
...
function findFirstAvailableTrackIndex({
	...
	return tracks.findIndex((track) => {
		return (
			areTrackTypesCompatible(track.type, trackType) &&
...
export function resolveTrackPlacement({
	...
	if (strategy.type === "explicit") {
		...
		if (!areTrackTypesCompatible(track.type, trackType)) {
			return null;
		}
	...
	if (strategy.type === "preferIndex") {
		...
		const isPreferredTrackCompatible =
			!!preferredTrack && areTrackTypesCompatible(preferredTrack.type, trackType);
	...
	if (strategy.type === "aboveSource") {
		...
		if (
			aboveTrack &&
			areTrackTypesCompatible(aboveTrack.type, trackType) &&
			canPlaceTimeSpansOnTrack({
				track: aboveTrack,
				timeSpans,
			})
