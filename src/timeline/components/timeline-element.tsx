...
import { addMediaTime, type MediaTime, TICKS_PER_SECOND, ZERO_MEDIA_TIME } from "@/wasm";
import {
	getActionDefinition,
	type TAction,
...
export function TimelineElement({
	...
	const isCurrentElementSelected = isText
		? selectedElements.some((selected) => selected.trackId === track.id)
	...
	const dragTimeOffset = isDragging && !isText
	...
	const timelineDuration = useEditor((e) => e.timeline.getTotalDuration());

	const displayedStartTime = isText
		? ZERO_MEDIA_TIME
		: isDragging && isBeingDragged && dragTimeOffset !== undefined
		? addMediaTime({ a: dragView.currentTime, b: dragTimeOffset })
		: renderElement.startTime;
	const displayedDuration = isText
		? timelineDuration
...
function ElementInner({
	...
	return (
		<div
			...
			style={{
				left: isText ? "0px" : `${ELEMENT_RING_WIDTH_PX}px`,
				right: isText ? "0px" : `${ELEMENT_RING_WIDTH_PX}px`,
			}}
		>
			<div
				className={cn("absolute inset-0", isText ? "rounded-none" : "rounded-sm")}
				style={
					isSelected
						? {
			...
			>
				<div
					className={cn(
						...
						isText ? "rounded-none" : "rounded-sm",
...
function TextElementContent({
	...
	const isSubtitleTrack = track.type === "text";
	return (
		<div className="flex size-full items-center justify-start pl-2">
			<span className="truncate text-[11px] font-semibold tracking-wide text-white">
				{isSubtitleTrack ? "Captions" : (typeof element.params.content === "string" ? element.params.content : "")}
			</span>
		</div>
	);
}
...
function ElementContent({ element, track }: ElementContentProps) {
	switch (element.type) {
		case "text":
			return <TextElementContent element={element} track={track} />;
		case "effect":
