...
import { getTrackTypeForElementType, areTrackTypesCompatible } from "@/timeline/placement/compatibility";
...
function findCompatibleTrackPlacement({
	...
	for (
		...
		if (
			placement.trackType !== "text" &&
			requiredTrackType !== "text" &&
			areTrackTypesCompatible(placement.trackType, requiredTrackType) &&
