...
export function measureTextBlock({
	...
	let actualLeft = 0;
	let actualRight = 0;
	let actualTop = -height / 2;
	let actualBottom = height / 2;
	...
	if (lineMetrics.length > 0) {
		...
		for (let index = 0; index < lineMetrics.length; index++) {
			...
			// actualBoundingBoxLeft represents distance going left from alignment point
			const boxLeft = metrics.actualBoundingBoxLeft ?? 0;
			const boxRight = metrics.actualBoundingBoxRight ?? metrics.width;
			// actualBoundingBoxAscent/Descent are relative to middle baseline
			const boxAscent = metrics.actualBoundingBoxAscent ?? lineHeightPx * 0.4;
			const boxDescent = metrics.actualBoundingBoxDescent ?? lineHeightPx * 0.1;

			const leftBoundary = -boxLeft;
			const rightBoundary = boxRight;
			const topBoundary = lineY - boxAscent;
			const bottomBoundary = lineY + boxDescent;

			if (leftBoundary < minLeft) minLeft = leftBoundary;
			if (rightBoundary > maxRight) maxRight = rightBoundary;
			if (topBoundary < minTop) minTop = topBoundary;
			if (bottomBoundary > maxBottom) maxBottom = bottomBoundary;
		}

		actualLeft = minLeft;
		actualRight = maxRight;
		actualTop = minTop;
...
function getTextRect({
	...
	if (
		block.actualLeft !== undefined &&
		block.actualTop !== undefined &&
		block.actualWidth !== undefined &&
		block.actualHeight !== undefined
	) {
		return {
			left: block.actualLeft,
			top: block.actualTop,
