...
function resolveTextNode({
	...
	const durationSeconds = mediaTimeToSeconds({ time: node.params.duration });
	const karaokeEnabled = node.params.params["karaoke.enabled"] === true;
	const karaokeProgress = durationSeconds > 0 ? Math.max(0, Math.min(1, (localTime / TICKS_PER_SECOND) / durationSeconds)) : 0;

	const fontFamily = typeof node.params.params.fontFamily === "string" ? node.params.params.fontFamily : "Arial";
	if (typeof window !== "undefined" && !SYSTEM_FONTS.has(fontFamily)) {
		loadFullFont({ family: fontFamily }).catch(() => {});
	}
	...
	return {
		...
		strokeEnabled: node.params.params["stroke.enabled"] === true,
		strokeColor: typeof node.params.params["stroke.color"] === "string" ? node.params.params["stroke.color"] : "#000000",
		strokeWidth: typeof node.params.params["stroke.width"] === "number" ? node.params.params["stroke.width"] : 4,
		karaokeEnabled,
		karaokeHighlightColor: typeof node.params.params["karaoke.highlightColor"] === "string" ? node.params.params["karaoke.highlightColor"] : "#00ff00",
		karaokeBaseColor: typeof node.params.params["karaoke.baseColor"] === "string" ? node.params.params["karaoke.baseColor"] : "#555555",
		karaokeProgress,
		effectPasses: resolveEffectPassGroups({
