...
class WasmCompositor {
	...
	ensureInitialized({ width, height }: { width: number; height: number }) {
		if (!this.canvas) {
			// Force WebGL fallback by hiding navigator.gpu to prevent wgpu crashes
	...
	private syncExternalTexture(texture: ExternalTextureDescriptor) {
		...
		if (
			...
			JSON.stringify(previous.crop) === JSON.stringify(texture.crop)
		) {
...
function ensureOffscreenCanvas({
	...
	if (source instanceof OffscreenCanvas && !crop) {
		return source;
	}
