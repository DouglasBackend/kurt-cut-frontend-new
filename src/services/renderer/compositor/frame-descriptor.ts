...
import { isFontLoaded } from "@/fonts/google-fonts";
...
async function collectVisualSourceNode({
	...
	const scaleX = Math.abs(node.resolved.transform.scaleX);
	const scaleY = Math.abs(node.resolved.transform.scaleY);
	const contentScale = node.resolved.transform.contentScale;
	const uniformScale = Math.max(contentScale ?? 0, scaleX, scaleY) || 1;
	const cropWidth = sourceWidth * (scaleX / uniformScale);
	const cropHeight = sourceHeight * (scaleY / uniformScale);
...
function collectTextNode({
	...
	const fontIsLoaded = isFontLoaded(fontFamily);
