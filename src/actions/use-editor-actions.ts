	...
	useActionHandler(
		...
		() => {
			...
			const elementsToSplit =
				selectedElements.length > 0
					? selectedElements.map((ref) => {
							const track = tracks.overlay.find((t) => t.id === ref.trackId);
							if (track && track.type === "text") {
								const elementUnderPlayhead = track.elements.find(
									(el) =>
										currentTime >= el.startTime &&
										currentTime <= el.startTime + el.duration,
								);
								if (elementUnderPlayhead) {
									return { trackId: track.id, elementId: elementUnderPlayhead.id };
								}
							}
							return ref;
						})
			...
			switch (editor.selection.getActiveSelectionKind()) {
				...
				case "elements": {
					...
					for (const ref of selectedElements) {
						const track = scene.tracks.overlay.find((t) => t.id === ref.trackId);
						if (track && track.type === "text") {
							for (const el of track.elements) {
								if (!elementsToDelete.some((e) => e.elementId === el.id)) {
									elementsToDelete.push({ trackId: track.id, elementId: el.id });
								}
							}
						}
					}
	...
	useActionHandler(
		...
		() => {
			...
			for (const ref of selectedElements) {
				...
				if (track && track.type === "text") {
					for (const el of track.elements) {
						if (!elementsToDuplicate.some((e) => e.elementId === el.id)) {
							elementsToDuplicate.push({ trackId: track.id, elementId: el.id });
						}
					}
				}
			}
