	...
	return (
		<div className="panel bg-background flex h-full overflow-hidden rounded-lg border">
			<TooltipProvider delayDuration={0}>
				<div className="flex shrink-0 flex-col gap-1 border-r p-1.5 scrollbar-hidden overflow-y-auto">
					{visibleTabs.map((tab) => (
						<Tooltip key={tab.id}>
							<TooltipTrigger asChild>
								<Button
									...
									className={cn(
										"shrink-0 rounded-md transition-colors duration-150",
										"h-8 w-8",
										tab.id !== activeTab.id && "text-muted-foreground hover:text-foreground",
									)}
								>
									{tab.icon}
								</Button>
