...
function getPreferredEdge({ edge }: { edge: Edge }): ScaleEdgePreference {
	if (edge === "right") return { right: true };
	if (edge === "left") return { left: true };
	if (edge === "top") return { top: true };
	return { bottom: true };
}
...
export class TransformHandleController {
	...
	private previewCornerScale({
		...
		const initialContentScale = typeof session.initialParams["transform.contentScale"] === "number"
			? session.initialParams["transform.contentScale"]
			: Math.max(Math.abs(session.initialTransform.scaleX), Math.abs(session.initialTransform.scaleY));
		const newContentScale = initialContentScale * snappedScale;
	...
	private previewEdgeScale({
		...
		const proposedScaleY =
			session.edge === "bottom" || session.edge === "top"
				? proposedScale
...
function buildParamsWithTransform({
	...
	return {
		...
		...(contentScale !== undefined && { "transform.contentScale": contentScale }),
	};
}
