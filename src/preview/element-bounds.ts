...
function getElementBounds({
	...
	if (element.type === "text") {
		...
		const padding = 16;
		const paddedRect = {
			left: measured.visualRect.left - padding,
			top: measured.visualRect.top - padding,
			width: measured.visualRect.width + padding * 2,
			height: measured.visualRect.height + padding * 2,
		};
...
export function getEdgeHandlePosition({
	...
	const localY = edge === "bottom" ? halfHeight : edge === "top" ? -halfHeight : 0;
	return {
		x: bounds.cx + (localX * cos - localY * sin),
		y: bounds.cy + (localX * sin + localY * cos),
	};
