...
export function SoundsView() {
	const { setSearchQuery, setSearchResults, setHasLoaded } = useSoundsStore();
	const [activeTab, setActiveTab] = useState<"sound-effects" | "songs" | "saved">("sound-effects");

	const handleTabChange = (val: string) => {
		setActiveTab(val as any);
		// Reset search query and search results, and mark as not loaded so it fetches the new tab's top list
	...
	return (
		<div className="flex h-full flex-col">
			<Tabs value={activeTab} onValueChange={handleTabChange} className="flex h-full flex-col">
				...
				>
					<AudioListView type="effects" />
				</TabsContent>
				<TabsContent
					value="songs"
					className="mt-0 flex min-h-0 flex-1 flex-col p-5 pt-0"
				>
					<AudioListView type="songs" />
				</TabsContent>
				<TabsContent
					value="saved"
					className="mt-0 flex min-h-0 flex-1 flex-col p-5 pt-0"
...
function AudioListView({ type }: { type: "effects" | "songs" }) {
	const {
		topSoundEffects,
		isLoading,
		searchQuery,
		...
		const fetchTopSounds = async () => {
			try {
				...
				const response = await fetch(
					`/api/sounds/search?page_size=50&sort=downloads&type=${type}`,
				);
	...
	return (
		<div className="mt-1 flex h-full flex-col gap-5">
			<div className="flex items-center gap-3">
				<Input
					placeholder={type === "songs" ? "Buscar músicas..." : "Buscar efeitos sonoros..."}
					className="w-full"
				...
				<DropdownMenu>
					...
					<DropdownMenuContent align="end" className="w-56">
						...
						>
							Apenas licença comercial
						</DropdownMenuCheckboxItem>
						<div className="text-muted-foreground px-2 py-1.5 text-xs">
							{showCommercialOnly
								? "Mostrando apenas áudios licenciados para uso comercial"
								: "Mostrando todos os áudios, independente da licença"}
						</div>
					</DropdownMenuContent>
				</DropdownMenu>
			</div>
				...
				>
					<div className="flex flex-col gap-4">
						...
						{!isLoading && !isSearching && displayedSounds.length === 0 && (
							<div className="text-muted-foreground text-sm">
								{searchQuery ? "Nenhum áudio encontrado" : "Nenhum áudio disponível"}
							</div>
						)}
						{isLoadingMore && (
		...
		return (
			<div className="bg-background flex h-full flex-col items-center justify-center gap-3 p-4">
				...
				<div className="flex flex-col gap-2 text-center">
					<p className="text-lg font-medium">Nenhum áudio salvo</p>
					<p className="text-muted-foreground text-sm text-balance">
						Clique no ícone de coração em qualquer som para salvá-lo aqui
					</p>
				</div>
			</div>
		);
		...
		<div className="mt-1 flex h-full flex-col gap-5">
			<div className="flex items-center justify-between">
				<p className="text-muted-foreground text-sm">
					{savedSounds.length} {savedSounds.length === 1 ? "áudio salvo" : "áudios salvos"}
				</p>
				<Dialog open={showClearDialog} onOpenChange={setShowClearDialog}>
					...
					<DialogContent>
						<DialogHeader>
							<DialogTitle>Limpar todos os áudios salvos?</DialogTitle>
							<DialogDescription>
								Isso removerá permanentemente todos os {savedSounds.length} áudios salvos da sua coleção. Esta ação não pode ser desfeita.
							</DialogDescription>
						</DialogHeader>
						<DialogFooter>
		...
		<div className="group flex items-center gap-3 opacity-100 hover:opacity-75">
			...
			<div className="flex items-center gap-3 pr-2">
				...
				<Button
					...
					title={isSaved ? "Remover dos salvos" : "Salvar áudio"}
				>
					<HugeiconsIcon
						icon={FavouriteIcon}
